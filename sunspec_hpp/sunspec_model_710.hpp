...
public:
    static constexpr uint16_t ID = 710;
    Model710_Raw raw;
    ...
    void print_attributes() const override {
        ...
        const uint8_t* cur_ptr = base_addr + sizeof(Model710_Raw);
        // Loop for group: Crv
        for (size_t i = 0; i < be16toh_custom(raw.NCrvSet); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model710_Crv_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model710_Crv_Raw*>(cur_ptr);
            std::cout << "    Group Crv[" << i << "]:" << std::endl;
            std::cout << "    ReadOnly: " << be16toh_custom(grp->ReadOnly) << std::endl;
            cur_ptr += sizeof(Model710_Crv_Raw);
        // Group: MustTrip (Single)
        if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MustTrip_Raw) <= (size_t)(get_raw_L() * 2 + 4)) {
            auto* grp = reinterpret_cast<const Model710_Crv_MustTrip_Raw*>(cur_ptr);
            std::cout << "    Group MustTrip:" << std::endl;
            std::cout << "    ActPt: " << be16toh_custom(grp->ActPt) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MustTrip_Raw);
        // Loop for group: Pt
        for (size_t i = 0; i < be16toh_custom(raw.NPt); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MustTrip_Pt_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model710_Crv_MustTrip_Pt_Raw*>(cur_ptr);
            std::cout << "    Group Pt[" << i << "]:" << std::endl;
            std::cout << "    Hz: " << be32toh_custom(grp->Hz) << std::endl;
            std::cout << "    Tms: " << be32toh_custom(grp->Tms) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MustTrip_Pt_Raw);
        }
        }
        // Group: MayTrip (Single)
        if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MayTrip_Raw) <= (size_t)(get_raw_L() * 2 + 4)) {
            auto* grp = reinterpret_cast<const Model710_Crv_MayTrip_Raw*>(cur_ptr);
            std::cout << "    Group MayTrip:" << std::endl;
            std::cout << "    ActPt: " << be16toh_custom(grp->ActPt) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MayTrip_Raw);
        // Loop for group: Pt
        for (size_t i = 0; i < be16toh_custom(raw.NPt); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MayTrip_Pt_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model710_Crv_MayTrip_Pt_Raw*>(cur_ptr);
            std::cout << "    Group Pt[" << i << "]:" << std::endl;
            std::cout << "    Hz: " << be32toh_custom(grp->Hz) << std::endl;
            std::cout << "    Tms: " << be32toh_custom(grp->Tms) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MayTrip_Pt_Raw);
        }
        }
        // Group: MomCess (Single)
        if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MomCess_Raw) <= (size_t)(get_raw_L() * 2 + 4)) {
            auto* grp = reinterpret_cast<const Model710_Crv_MomCess_Raw*>(cur_ptr);
            std::cout << "    Group MomCess:" << std::endl;
            std::cout << "    ActPt: " << be16toh_custom(grp->ActPt) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MomCess_Raw);
        // Loop for group: Pt
        for (size_t i = 0; i < be16toh_custom(raw.NPt); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model710_Crv_MomCess_Pt_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model710_Crv_MomCess_Pt_Raw*>(cur_ptr);
            std::cout << "    Group Pt[" << i << "]:" << std::endl;
            std::cout << "    Hz: " << be32toh_custom(grp->Hz) << std::endl;
            std::cout << "    Tms: " << be32toh_custom(grp->Tms) << std::endl;
            cur_ptr += sizeof(Model710_Crv_MomCess_Pt_Raw);
        }
        }
        }
    }
