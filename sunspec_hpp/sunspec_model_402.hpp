...
public:
    static constexpr uint16_t ID = 402;
    Model402_Raw raw;
    ...
    void print_attributes() const override {
        ...
        const uint8_t* cur_ptr = base_addr + sizeof(Model402_Raw);
        {
            size_t rem_bytes = (get_raw_L() * 2 + 4) - (size_t)(cur_ptr - base_addr);
            size_t count = rem_bytes / sizeof(Model402_string_Raw);
        // Loop for group: string
        for (size_t i = 0; i < count; ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model402_string_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model402_string_Raw*>(cur_ptr);
            ...
            cur_ptr += sizeof(Model402_string_Raw);
        }
        }
    }
