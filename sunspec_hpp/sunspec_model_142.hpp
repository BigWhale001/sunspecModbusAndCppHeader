// Generated C++ Header for Model 142
#pragma once
...
class Model142 : public SunSpecModelBase {
public:
    static constexpr uint16_t ID = 142;
    Model142_Raw raw;
    ...
    void from_buffer(const uint8_t* buffer) override {
        base_addr = buffer;
        std::memcpy(&raw, buffer, sizeof(Model142_Raw));
    }
    ...
    static const Model142_curve_Raw* get_curve(const uint8_t* base_buffer, size_t index, size_t offset_bytes) {
        return reinterpret_cast<const Model142_curve_Raw*>(base_buffer + offset_bytes + index * sizeof(Model142_curve_Raw));
    }
    ...
    void print_attributes() const override {
        ...
        const uint8_t* cur_ptr = base_addr + sizeof(Model142_Raw);
        {
            size_t rem_bytes = (get_raw_L() * 2 + 4) - (size_t)(cur_ptr - base_addr);
            size_t count = rem_bytes / sizeof(Model142_curve_Raw);
        // Loop for group: curve
        for (size_t i = 0; i < count; ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model142_curve_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model142_curve_Raw*>(cur_ptr);
            ...
            cur_ptr += sizeof(Model142_curve_Raw);
        }
        }
    }
