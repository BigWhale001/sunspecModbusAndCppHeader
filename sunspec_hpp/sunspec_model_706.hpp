...
public:
    ...
    void print_attributes() const override {
        ...
        const uint8_t* cur_ptr = base_addr + sizeof(Model706_Raw);
        // Loop for group: Crv
        for (size_t i = 0; i < be16toh_custom(raw.NCrv); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model706_Crv_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model706_Crv_Raw*>(cur_ptr);
            ...
            cur_ptr += sizeof(Model706_Crv_Raw);
        // Loop for group: Pt
        for (size_t i = 0; i < be16toh_custom(raw.NPt); ++i) {
            if ((cur_ptr - base_addr) + sizeof(Model706_Crv_Pt_Raw) > (size_t)(get_raw_L() * 2 + 4)) break;
            auto* grp = reinterpret_cast<const Model706_Crv_Pt_Raw*>(cur_ptr);
            std::cout << "    Group Pt[" << i << "]:" << std::endl;
            std::cout << "    V: " << be16toh_custom(grp->V) << std::endl;
            std::cout << "    W: " << be16toh_custom_s(grp->W) << std::endl;
            cur_ptr += sizeof(Model706_Crv_Pt_Raw);
        }
        }
    }
