// Generated C++ Header for Model 705
#pragma once
...
struct Model705_Crv_Raw {
    ...
    uint16_t VRef;
    uint16_t VRefAuto;
    uint16_t VRefAutoEna;
    uint16_t VRefAutoTms;
    uint32_t RspTms;
    uint16_t ReadOnly;
};
#pragma pack(pop)
...
class Model705 : public SunSpecModelBase {
public:
    Model705_Raw raw;

    uint16_t get_id() const override { return 705; }

    void from_buffer(const uint8_t* buffer) override {
        base_addr = buffer;
        std::memcpy(&raw, buffer, sizeof(Model705_Raw));
    }
    ...
    static const Model705_Crv_Raw* get_Crv(const uint8_t* base_buffer, size_t index, size_t offset_bytes) {
        return reinterpret_cast<const Model705_Crv_Raw*>(base_buffer + offset_bytes + index * sizeof(Model705_Crv_Raw));
    }

    // Accessor for repeating group: Crv_Pt
    static const Model705_Crv_Pt_Raw* get_Crv_Pt(const uint8_t* base_buffer, size_t index, size_t offset_bytes) {
        return reinterpret_cast<const Model705_Crv_Pt_Raw*>(base_buffer + offset_bytes + index * sizeof(Model705_Crv_Pt_Raw));
    }
