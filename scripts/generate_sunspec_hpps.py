...
CPP_KEYWORDS = [
    'int', 'float', 'char', 'long', 'short', 'double', 
    'struct', 'class', 'public', 'private', 'protected', 
    'virtual', 'override', 'default', 'case', 'switch', 
    'if', 'else', 'for', 'while', 'do', 'break', 'continue', 
    'return', 'void', 'static', 'const', 'constexpr'
]
...
def get_conversion_func(ptype):
    if '32' in ptype:
        return 'be32toh_custom'
    elif ptype in ['int16', 'sunssf']:
        return 'be16toh_custom_s'
    return 'be16toh_custom'
...
def has_repeating_groups(group):
    for sub in group.get('groups', []):
        count = sub.get('count', 1)
        # If count is not exactly integer 1, it's repeating (0, >1, or string)
        if not (isinstance(count, int) and count == 1):
            return True
        # If sub has repeating groups, then group has repeating groups
        if has_repeating_groups(sub):
            return True
    return False

def collect_points_with_single_groups(group, prefix=""):
    points = []
    for p in group.get('points', []):
        p_copy = dict(p)
        if prefix:
            p_copy['name'] = f"{prefix}_{p_copy['name']}"
        points.append(p_copy)
    for sub in group.get('groups', []):
        count = sub.get('count', 1)
        if isinstance(count, int) and count == 1:
            # Only flatten if it doesn't contain repeating groups
            if not has_repeating_groups(sub):
                sub_prefix = clean_name(sub['name']) if not prefix else f"{prefix}_{clean_name(sub['name'])}"
                points.extend(collect_points_with_single_groups(sub, sub_prefix))
    return points

def print_points(h, points, accessor_prefix):
    for p in points:
        name = clean_name(p['name'])
        ptype = p['type']
        size = p.get('size', 1)
        ...
        conv = get_conversion_func(ptype)
            
        if ptype == 'string':
            h.write(f'            std::cout << "    {name}: ";\n')
            h.write(f'            for(size_t i=0; i<sizeof({accessor_prefix}{name}) && {accessor_prefix}{name}[i] != 0; ++i) std::cout << {accessor_prefix}{name}[i];\n')
            h.write('            std::cout << std::endl;\n')
        elif size > 1 and '32' not in ptype: # Array
            h.write(f'            std::cout << "    {name}: [";\n')
            h.write(f'            for(size_t i=0; i<{size}; ++i) {{\n')
            h.write(f'                std::cout << {conv}({accessor_prefix}{name}[i]) << (i < {size-1} ? ", " : "");\n')
            h.write('            }\n')
            h.write('            std::cout << "]" << std::endl;\n')
        else: # Scalar
            h.write(f'            std::cout << "    {name}: " << {conv}({accessor_prefix}{name}) << std::endl;\n')

def get_count_expression(count, parent_source, raw_points, current_group_points):
    """
    Resolve count expression for C++.
    count: integer, string (point name), or 0.
    parent_source: C++ accessor for current group (e.g., "(*grp)" or "raw").
    raw_points: list of points in the main raw struct.
    current_group_points: list of points in the current group struct.
    """
    if isinstance(count, int):
        if count == 0:
            return None # Special handling needed (calc from length)
        return str(count)
    
    if isinstance(count, str):
        c_name = clean_name(count)
        
        # 1. Check if in current group
        for p in current_group_points:
            if clean_name(p['name']) == c_name:
                # Found in current group
                ptype = p['type']
                conv = get_conversion_func(ptype)
                return f"{conv}({parent_source}.{c_name})"
        
        # 2. Check if in raw (global)
        for p in raw_points:
            if clean_name(p['name']) == c_name:
                ptype = p['type']
                conv = get_conversion_func(ptype)
                return f"{conv}(raw.{c_name})"

    return "0" # Fallback

def generate_group_printing(h, groups, prefix, parent_source, model_id, raw_points):
    for grp in groups:
        count = grp.get('count', 1)
        name = grp['name']
        clean_gname = clean_name(name)
        full_gname = f"{prefix}_{clean_gname}" if prefix else clean_gname
        
        is_single = (isinstance(count, int) and count == 1)
        
        if is_single and not has_repeating_groups(grp):
            sub_groups = grp.get('groups', [])
            if sub_groups:
                # Recurse
                generate_group_printing(h, sub_groups, full_gname, parent_source, model_id, raw_points)
            continue
            
        # Non-flattened (Repeating OR Single with Repeating Subgroups)
        struct_name = f"Model{model_id}_{full_gname}_Raw"
        g_points = collect_points_with_single_groups(grp)
        
        if is_single:
             # Single Group Logic (No Loop)
             h.write(f"        // Group: {name} (Single)\n")
             h.write(f"        if ((cur_ptr - base_addr) + sizeof({struct_name}) <= (size_t)(get_raw_L() * 2 + 4)) {{\n")
             h.write(f"            auto* grp = reinterpret_cast<const {struct_name}*>(cur_ptr);\n")
             h.write(f"            std::cout << \"    Group {name}:\" << std::endl;\n")
             
             print_points(h, g_points, "grp->")
             
             h.write(f"            cur_ptr += sizeof({struct_name});\n")
             
             # Nested groups
             sub_groups = grp.get('groups', [])
             if sub_groups:
                 generate_group_printing(h, sub_groups, full_gname, "(*grp)", model_id, raw_points)
        ...
        if count == 0:
             h.write(f"        {{\n")
             h.write(f"            size_t rem_bytes = (get_raw_L() * 2 + 4) - (size_t)(cur_ptr - base_addr);\n")
             h.write(f"            size_t count = rem_bytes / sizeof({struct_name});\n")
             count_expr = "count"
        else:
             expr = get_count_expression(count, parent_source, raw_points, g_points)
             if expr:
                 count_expr = expr
             else:
                 count_expr = "0"
        
        h.write(f"        // Loop for group: {name}\n")
        h.write(f"        for (size_t i = 0; i < {count_expr}; ++i) {{\n")
        h.write(f"            if ((cur_ptr - base_addr) + sizeof({struct_name}) > (size_t)(get_raw_L() * 2 + 4)) break;\n")
        h.write(f"            auto* grp = reinterpret_cast<const {struct_name}*>(cur_ptr);\n")
        h.write(f"            std::cout << \"    Group {name}[\" << i << \"]:\" << std::endl;\n")
...
def process_subgroups(h, model_id, groups, prefix, all_subgroups):
    for grp in groups:
        ...
        if is_single and not has_repeating_groups(grp):
            # Flattened group: recurse but don't generate struct
            if sub_groups:
                process_subgroups(h, model_id, sub_groups, full_gname, all_subgroups)
        else:
            # Repeating Group OR Single Group with repeating subgroups
            g_struct_name = f"Model{model_id}_{full_gname}_Raw"
            # Use collect_points_with_single_groups to flatten nested SAFE count=1 groups into this struct
            g_points = collect_points_with_single_groups(grp)
            
            h.write(f"// Group: {full_gname}\n")
...
def generate_cpp_header(json_path, output_dir):
    ...
    points = collect_points_with_single_groups(group_data)
    ...
    with open(header_path, 'w', encoding='utf-8') as h:
        ...
        for f in fields:
            ...
            if ptype in ['uint16', 'int16', 'enum16', 'bitfield16']:
                if sf:
                    ...
                    if ptype.startswith('u'):
                        h.write(f"        uint16_t val = {conv}(raw.{name});\n")
                    else:
                        h.write(f"        int16_t val = {conv}(raw.{name});\n")
            ...
            if ptype in ['uint16', 'int16', 'enum16', 'bitfield16', 'sunssf']:
                ...
                conv = get_conversion_func(ptype)
                h.write(f"        return {conv}(raw.{name});\n")
                h.write(f"    }}\n\n")
        ...
        print_points(h, points, "raw.")
                
        h.write(f"        const uint8_t* cur_ptr = base_addr + sizeof({struct_name});\n")
        generate_group_printing(h, subgroups, "", "raw", model_id, points)
