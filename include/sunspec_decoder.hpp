...
#include "sunspec_hpp/sunspec_model_base.hpp"
...
public:
    ...
    // Helper to access nested group structure from a model's base address
    template <typename T>
    static const T* get_group_at(const std::shared_ptr<SunSpecModelBase>& model, size_t byte_offset) {
        if (!model || !model->base_addr) return nullptr;
        return reinterpret_cast<const T*>(model->base_addr + byte_offset);
    }
    ...
    void decode(const uint16_t* buffer, size_t length) {
        ...
        while (cursor + 1 < length) {
            ...
            uint16_t len = (buffer[cursor + 1] >> 8) | (buffer[cursor + 1] << 8);
